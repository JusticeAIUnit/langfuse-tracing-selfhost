...
**🛠️ How-to Guide** - This guide helps you diagnose and resolve issues specific to ClickHouse replication and ZooKeeper coordination in LangFuse deployments.
:::
...
SELECT 
  ...
  version,
  dirty,
  CASE WHEN dirty = 1 THEN '⚠️ DIRTY' ELSE '✅ CLEAN' END as status
FROM schema_migrations 
ORDER BY sequence;"
```
...
for uuid in $UUIDS; do
  ...
  echo "  ✓ Cleaned metadata for UUID: $uuid"
done
...
for replica in langfuse-clickhouse-shard0-1 langfuse-clickhouse-shard0-2; do
  ...
  for table in "${REPLICATED_TABLES[@]}"; do
    ...
    echo "    ✓ Synchronized $table on $replica"
  done
done
...
for pod in langfuse-clickhouse-shard0-{0,1,2}; do
  ...
  if [ ${#missing_tables[@]} -gt 0 ]; then
    echo "  ⚠️  Missing tables on $pod: ${missing_tables[*]}"
  fi
  echo
done
```
...
for table in "${REPLICATED_TABLES[@]}"; do
  ...
  if [ "${counts[0]}" = "${counts[1]}" ] && [ "${counts[1]}" = "${counts[2]}" ] && [ "${counts[0]}" != "ERROR" ]; then
    echo "  ✓ Data consistent across all replicas"
  else
    echo "  ✗ Data inconsistency detected - split-brain issue!"
  fi
  echo
done
```
