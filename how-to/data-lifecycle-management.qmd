...
**🛠️ How-to Guide** - This guide helps you implement Time To Live (TTL) rules for automatic data lifecycle management in your LangFuse deployment, ensuring compliance with data protection retention policies.
:::
...
### Step 4: Multi-Shard Deployment Considerations

::: {.callout-warning}
**Critical for Replicated Environments**: In multi-shard ClickHouse deployments, TTL changes must be applied to **ALL shards** to ensure consistency.
:::

**Single Shard Application:**
```sql
-- Connect to one shard
kubectl exec -it langfuse-clickhouse-shard0-0 -n langfuse -- clickhouse-client
ALTER TABLE traces MODIFY TTL toDateTime(timestamp) + INTERVAL 30 DAY DELETE WHERE project_id = 'YOUR_PROJECT_ID';
```

**Multi-Shard Application (Recommended):**
```bash
# Apply to all shards in your deployment
...
**Check Current TTL Rules:**
```sql
-- View complete table structure including TTL
SHOW CREATE TABLE table_name;

-- Query system tables for TTL information
SELECT table, ttl_expression 
FROM system.tables 
WHERE database = 'default' AND table IN ('traces', 'observations', 'scores');
```

**Multi-Shard Verification Script:**
```bash
#!/bin/bash
echo "Verifying TTL configuration across all shards..."
for pod in langfuse-clickhouse-shard0-0 langfuse-clickhouse-shard0-1 langfuse-clickhouse-shard0-2; do
  echo "--- Checking $pod ---"
  kubectl exec "$pod" -n langfuse -- clickhouse-client \
    --password="$CH_PASSWORD" -q "SHOW CREATE TABLE traces" | grep "TTL" || echo "No TTL found"
done
```

### Step 6: Force Immediate Cleanup (Optional)
...
### Selective TTL Removal

Sometimes you need to remove TTL rules entirely for specific tables while maintaining them for others. This is useful when certain data needs indefinite retention for compliance or analysis purposes.

**Remove TTL Completely:**
```sql
ALTER TABLE scores REMOVE TTL;
```

**Example - Mixed Retention Policy:**
```sql
-- Keep traces and observations with 3-day TTL for data quality
ALTER TABLE traces 
MODIFY TTL 
    toDateTime(timestamp) + INTERVAL 3 DAY 
    DELETE WHERE project_id = 'YOUR_PROJECT_ID';
...
ALTER TABLE observations 
MODIFY TTL 
    toDateTime(start_time) + INTERVAL 3 DAY 
    DELETE WHERE project_id = 'YOUR_PROJECT_ID';

-- Remove TTL from scores for indefinite retention
ALTER TABLE scores REMOVE TTL;
```
...
**Problem**: Need to completely remove TTL rules from a table
...
**Verification**:
```bash
# Verify TTL removal across all shards
for pod in langfuse-clickhouse-shard0-0 langfuse-clickhouse-shard0-1 langfuse-clickhouse-shard0-2; do
  echo "--- Checking $pod ---"
  kubectl exec "$pod" -n langfuse -- clickhouse-client \
    -q "SHOW CREATE TABLE scores" | grep "TTL" || echo "No TTL found"
done
```

### Multi-Shard Inconsistency

**Problem**: TTL rules applied to only one shard, causing data inconsistency

**Symptoms**:
- Inconsistent data counts between UI refreshes
- Some data appears and disappears randomly
- Query results vary depending on which shard responds

**Solution**:
```bash
# Always apply TTL changes to ALL shards
for pod in langfuse-clickhouse-shard0-0 langfuse-clickhouse-shard0-1 langfuse-clickhouse-shard0-2; do
  kubectl exec "$pod" -n langfuse -- clickhouse-client \
    --password="$CH_PASSWORD" -q "ALTER TABLE table_name REMOVE TTL;"
done
```

### Data Retention Monitoring

**Problem**: TTL rules not working as expected or data growing unexpectedly

**Diagnostic Queries**:
```sql
-- Check data retention for tables WITH TTL (should return 0 for old data)
SELECT count() FROM traces 
WHERE timestamp < now() - INTERVAL 35 DAY  -- 5 days past 30-day TTL
  AND project_id = 'YOUR_PROJECT_ID';

-- Monitor growth for tables WITHOUT TTL
SELECT 
    project_id,
...
- **Multi-Shard Verification** - Always verify TTL configuration across all shards

**Recovery** (if backups exist):

```sql
-- Remove TTL rule (use correct syntax)
ALTER TABLE traces REMOVE TTL;
...
5. **Multi-Shard Awareness** - Always consider multi-shard deployments in TTL planning
6. **Syntax Verification** - Use `REMOVE TTL` for complete removal, `MODIFY TTL` for changes
...
5. **Multi-Shard Consistency** - Regularly verify TTL configuration across all shards
6. **Mixed Policy Management** - Monitor storage growth for tables with removed TTL
...
When implementing mixed TTL policies (some tables with TTL, others without):
...
**Tables WITHOUT TTL** (indefinite retention):

- Continuous storage growth - monitor disk usage
- Potential query performance degradation over time
- Increased backup storage requirements

**Monitoring Strategy**:
```sql
-- Monitor storage growth for tables without TTL
SELECT 
    table,
    formatReadableSize(total_bytes) as size,
    rows,
    formatDateTime(max_date, '%Y-%m-%d') as latest_data
FROM system.parts 
WHERE database = 'default' 
  AND table IN ('scores')  -- Tables without TTL
GROUP BY table;
```
...
```sql
-- Monitor query performance on growing tables
SELECT 
    query_duration_ms,
    read_rows,
    read_bytes,
...
WHERE query LIKE '%scores%'  -- Table without TTL
  AND type = 'QueryFinish'
ORDER BY event_time DESC 
LIMIT 10;
```

**Optimization Strategies for Indefinite Retention**:

- Implement date-based partitioning for easier management
- Consider archival strategies for very old data
- Monitor and optimize indexes as tables grow
- Plan for storage tier management (hot/warm/cold storage)

### Compliance and Risk Management

**Benefits of Mixed Policies**:

- ✅ **Compliance**: Critical data (scores) retained indefinitely for analysis
- ✅ **Data Quality**: Trace TTL prevents UI corruption from stale data
- ✅ **Storage Optimization**: Operational data still cleaned up automatically
- ✅ **Risk Mitigation**: Balanced approach to data retention
